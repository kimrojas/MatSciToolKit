...
## plot bands
plotlines = [None, None]
for i, (gnudata, color, label) in enumerate(zip([gnudata_up, gnudata_dn], ["k", "r"], ["Spin-up", "Spin-dn"])):
    for band in range(gnudata.shape[0]):
        plotlines[i] = ax.plot(gnudata[band, :, 0], gnudata[band, :, 1], color=color, linewidth=1, alpha=0.7, label=label)
        #  = line[0]

# Add the custom legend to the plot
ax.legend(
    handles=[plotlines[0][0], plotlines[1][0]],
    loc="upper left",
    fancybox=False,
    framealpha=1,
)
...
ax.set_xticks(processed_locations, labels=processed_labels)

## plot fermi energy
ax.axhline(efermi, color="b", linestyle="--", linewidth=0.5, alpha=0.5)

ax.set_ylabel("Energy (eV)")
ax.set_ylim(efermi - 10, efermi + 10)
